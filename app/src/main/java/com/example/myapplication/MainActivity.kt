...
@Composable
fun ResearchApp() {
    val navController = rememberNavController() // The navController is responsible for moving between screens.

    NavHost( // NavHost is like the screen container. It decides which screen should currently be visible.
        navController = navController,
        startDestination = Routes.PATIENT_LIST // Start the app at the patient list screen.
    ) {
        composable(Routes.PATIENT_LIST) {
            PatientListScreen(
                onPatientDetailClick = { patientId ->
        ...
        composable(route = "${Routes.PATIENT_DETAIL}/{patientId}") {
            backStackEntry ->
        ...
        composable(route = "${Routes.MEASUREMENT}/{sessionId}") {
            backStackEntry ->
        ...
        composable(route = "${Routes.RESULT}/{sessionId}") {
            backStackEntry ->
            val sessionId = backStackEntry.arguments
                ?.getString("sessionId")
                ?.toLongOrNull()
...
fun PatientListScreen(
    onPatientDetailClick: (Long) -> Unit // When clicks a patient, send the patient ID back to open patient detail.
) {
    Column(
        modifier = Modifier.padding(16.dp)
    ) {

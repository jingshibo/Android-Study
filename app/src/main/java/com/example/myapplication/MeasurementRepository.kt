...
class MeasurementRepository(
    ...
    suspend fun findPatientByCode(patientCode: String): PatientEntity? = patientDao.findPatientByCode(patientCode)
    ...
    suspend fun getOrCreatePatient(patient: PatientEntity): Long {
        val existing = patientDao.findPatientByCode(patient.patient_code)
        if (existing != null) {
    ...
    suspend fun searchPatients(
        ...
        fromCreatedAt: Long? = null,
        toCreatedAt: Long? = null
    ...
    suspend fun findDeviceByCode(deviceCode: String): DeviceEntity? = deviceDao.findDeviceByCode(deviceCode)
    ...
    suspend fun getOrCreateDevice(device: DeviceEntity): Long {
        val existing = deviceDao.findDeviceByCode(device.device_code)
        if (existing != null) {
    ...
    suspend fun searchDevices(
        ...
        minBatteryLevel: Int? = null,
        maxBatteryLevel: Int? = null,
        minMemoryAvailableByte: Long? = null,
        maxMemoryAvailableByte: Long? = null,
        ...
        minMemoryAvailableByte = minMemoryAvailableByte,
        maxMemoryAvailableByte = maxMemoryAvailableByte,
    ...
    suspend fun getOrCreateSession(session: SessionEntity): Long {
        val existing = sessionDao.findSessionByPatientDay(
    ...
    suspend fun searchSessions(
        ...
        fromRecordingDay: String? = null,
        toRecordingDay: String? = null,
        arm: ArmSide? = null,
        ...
        fromRecordingDay = fromRecordingDay,
        toRecordingDay = toRecordingDay,
        arm = arm?.name,
    ...
    suspend fun findSessionByPatientDay(
        patientId: Long,
        recordingDay: String
    ): SessionEntity? = sessionDao.findSessionByPatientDay(patientId, recordingDay)

    /** Retrieves a session matching a specific device ID and recording day. */
    suspend fun findSessionByDeviceAndDay(
        deviceId: Long,
        recordingDay: String
    ): SessionEntity? = sessionDao.findSessionByDeviceAndDay(deviceId, recordingDay)

    /** Retrieves all sessions matching a specific patient ID and device ID. */
    suspend fun findSessionsByPatientAndDevice(
        patientId: Long,
        deviceId: Long
    ): List<SessionEntity> = sessionDao.findSessionsByPatientAndDevice(patientId, deviceId)
    ...
    suspend fun searchMeasurements(
        ...
        fromRecordedAt: Long? = null,
        toRecordedAt: Long? = null,
        fromTransferredAt: Long? = null,
        toTransferredAt: Long? = null
    ...
    suspend fun findMeasurementByFileName(sessionId: Long, sensorFileName: String): MeasurementEntity? =
        measurementDao.findMeasurementByFileName(sessionId, sensorFileName)

    /** Diagnostic / Audit Function: Counts actual rows in the measurements table for a session 
     * to verify data integrity against SessionEntity.transferred_file_count. */
    ...
    suspend fun searchResults(
        ...
        fromExportedAt: Long? = null,
        toExportedAt: Long? = null
    ...
    suspend fun findLatestResultForSession(sessionId: Long): ResultEntity? =
        resultDao.findLatestResultBySession(sessionId)

    /** Checks if a prediction result exists for a specific session, model version, and preprocessing version. */
    suspend fun findResultByModelAndPreprocessing(
        sessionId: Long,
        modelVersion: String,
        preprocessingVersion: String? = null
    ): ResultEntity? = resultDao.findResultByModelAndPreprocessing(sessionId, modelVersion, preprocessingVersion)
...
interface PatientDao {
    ...
    @Query("SELECT * FROM patients WHERE patient_code = :patientCode")
    suspend fun findPatientByCode(patientCode: String): PatientEntity?
    ...
    WHERE (:patientCodeQuery IS NULL OR patient_code LIKE '%' || :patientCodeQuery || '%')
      ...
      AND (:toCreatedAt IS NULL OR created_at <= :toCreatedAt)
...
interface DeviceDao {
    ...
    @Query("SELECT * FROM devices WHERE device_code = :deviceCode")
    suspend fun findDeviceByCode(deviceCode: String): DeviceEntity?

    /** Retrieves a device matching the bluetooth address. */
    @Query("SELECT * FROM devices WHERE bluetooth_address = :bluetoothAddress")
    suspend fun findDeviceByBluetoothAddress(bluetoothAddress: String): DeviceEntity?
    ...
    WHERE (:deviceCodeQuery IS NULL OR device_code LIKE '%' || :deviceCodeQuery || '%')
      AND (:firmwareVersion IS NULL OR firmware_version LIKE '%' || :firmwareVersion || '%')
      AND (:minBatteryLevel IS NULL OR last_battery_level >= :minBatteryLevel)
      AND (:maxBatteryLevel IS NULL OR last_battery_level <= :maxBatteryLevel)
      AND (:minMemoryAvailableByte IS NULL OR last_memory_available_byte >= :minMemoryAvailableByte)
      AND (:maxMemoryAvailableByte IS NULL OR last_memory_available_byte <= :maxMemoryAvailableByte)
    ...
    suspend fun searchDevices(
        ...
        minBatteryLevel: Int?,
        maxBatteryLevel: Int?,
        minMemoryAvailableByte: Long?,
        maxMemoryAvailableByte: Long?,
        sensorStatus: String?
    ): List<DeviceEntity>
...
interface SessionDao {
    ...
    WHERE (:patientId IS NULL OR patient_id = :patientId)
      ...
      AND (:fromRecordingDay IS NULL OR recording_day >= :fromRecordingDay)
      AND (:toRecordingDay IS NULL OR recording_day <= :toRecordingDay)
    ...
    suspend fun searchSessions(
        ...
        deviceId: Long?,
        recordingDay: String?,
        fromRecordingDay: String?,
        toRecordingDay: String?,
        arm: String?,
...
interface MeasurementDao {
    ...
    @Query("SELECT * FROM measurements WHERE session_id = :sessionId AND sensor_file_name = :sensorFileName")
    suspend fun findMeasurementByFileName(sessionId: Long, sensorFileName: String): MeasurementEntity?
    ...
    WHERE (:sessionId IS NULL OR session_id = :sessionId)
      ...
      AND (:fromRecordedAt IS NULL OR recorded_at >= :fromRecordedAt)
      AND (:toRecordedAt IS NULL OR recorded_at <= :toRecordedAt)
      AND (:fromTransferredAt IS NULL OR transferred_at >= :fromTransferredAt)
      AND (:toTransferredAt IS NULL OR transferred_at <= :toTransferredAt)
...
interface ResultDao {
    ...
    suspend fun findLatestResultBySession(sessionId: Long): ResultEntity?

    /** Retrieves a prediction result matching exact session ID, model version, and preprocessing version. */
    @Query("SELECT * FROM results WHERE session_id = :sessionId AND model_version = :modelVersion AND (:preprocessingVersion IS NULL OR preprocessing_version = :preprocessingVersion)")
    suspend fun findResultByModelAndPreprocessing(sessionId: Long, modelVersion: String, preprocessingVersion: String?): ResultEntity?
    ...
    WHERE (:sessionId IS NULL OR session_id = :sessionId)
      ...
      AND (:fromExportedAt IS NULL OR exported_at >= :fromExportedAt)
      AND (:toExportedAt IS NULL OR exported_at <= :toExportedAt)
 ...
 *
 * IMPORTANT: Always validate these conditions in application logic BEFORE transferring or
